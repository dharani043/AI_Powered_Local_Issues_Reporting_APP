...
export function MunicipalityDashboard() {
  ...
  useEffect(() => {
    if (user?.role === 'municipality_admin') {
      // Filter issues by municipality ID for municipality admins
      let municipalityIssues = issues.filter(issue => 
        issue.municipalityId === user.municipalityId
      );
      
      // Apply multiple filters
      if (statusFilter !== 'all') {
        municipalityIssues = municipalityIssues.filter(issue => issue.status === statusFilter);
      }
      if (priorityFilter !== 'all') {
        municipalityIssues = municipalityIssues.filter(issue => issue.priority === priorityFilter);
      }
      if (categoryFilter !== 'all') {
        municipalityIssues = municipalityIssues.filter(issue => issue.category === categoryFilter);
      }
      if (locationFilter !== 'all') {
        municipalityIssues = municipalityIssues.filter(issue => {
      ...
      setFilteredIssues(municipalityIssues);
    } else {
      // Redirect non-municipality admins
      setFilteredIssues([]);
    }
  }, [issues, user, statusFilter, locationFilter, priorityFilter, categoryFilter]);

  const isWithinMunicipalityBounds = (geoLocation) => {
    // Simple bounds check - in production, use proper geofencing
  ...
  if (!user.municipalityId) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 dark:text-gray-400">Please contact system admin to configure your municipality</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-500 to-indigo-600 p-6 rounded-xl text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-100">Municipality ID: {user.municipalityId} | Pincode: {user.pincode}</p>
          </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Municipality Issues</h2>
        </div>
        ...
        <div className="divide-y divide-gray-200 dark:divide-gray-700">
          ...
          {filteredIssues.length === 0 && (
            <div className="p-12 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400">No issues found for your municipality</p>
            </div>
          )}
        </div>
      </div>
